...
class Connection extends events.EventEmitter {
  ...
  constructor(endpoint, protocolVersion, options) {
    ...
    if (options.sni) {
      ...
      this.endpoint = `${options.sni.addressResolver.getIp()}:${
        options.sni.port
      }`;
    ...
    this.port = Number(this.endpoint.substr(portSeparatorIndex + 1));
    ...
    if (!this.options.sslOptions) {
      ...
      this.netClient = tls.connect(
        ...
        function tlsConnectCallback() {
          self.log(
            "verbose",
            `Secure socket connected to ${
              self.endpointFriendlyName
            } with protocol ${self.netClient.getProtocol()}`
          );
          self.bindSocketListeners();
          self.startup(callback);
        }
